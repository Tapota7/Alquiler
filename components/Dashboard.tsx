...
const Dashboard: React.FC<DashboardProps> = ({ tenants, payments, owner, setActiveTab }) => {
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-500">
      <div>
        <h2 className="text-3xl font-bold tracking-tight text-slate-800 dark:text-white">Hola, de nuevo 👋</h2>
        <p className="text-slate-500 dark:text-slate-400 mt-1">Aquí tienes un resumen de tus alquileres para {new Intl.DateTimeFormat('es-ES', { month: 'long', year: 'numeric' }).format(new Date())}.</p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        <div className="bg-white dark:bg-slate-800 rounded-3xl border border-slate-100 dark:border-slate-700 shadow-sm overflow-hidden">
          <div className="p-6 border-b border-slate-50 dark:border-slate-700 flex items-center justify-between">
            <h3 className="font-bold text-lg text-slate-800 dark:text-white">Inquilinos Recientes</h3>
            <button onClick={() => setActiveTab('tenants')} className="text-indigo-600 dark:text-indigo-400 text-sm font-semibold flex items-center gap-1 hover:underline">
          ...
          <div className="divide-y divide-slate-50 dark:divide-slate-700">
            {tenants.length === 0 ? (
              ...
              tenants.slice(0, 5).map((tenant) => (
                <div key={tenant.id} className="p-4 flex items-center justify-between hover:bg-slate-50 dark:hover:bg-slate-700/50 transition-colors">
                  <div className="flex items-center gap-4">
                    <div className="w-12 h-12 rounded-2xl bg-indigo-50 dark:bg-indigo-900/40 flex items-center justify-center text-indigo-600 dark:text-indigo-400 font-bold">
                      {tenant.fullName.charAt(0)}
                    </div>
                    <div>
                      <p className="font-semibold text-slate-800 dark:text-white">{tenant.fullName}</p>
                      <p className="text-sm text-slate-500 dark:text-slate-400">{tenant.address}</p>
                    </div>
                  </div>
                  <div className="text-right">
                    <p className="font-bold text-slate-800 dark:text-white">{owner.currency}{tenant.monthlyRent.toLocaleString()}</p>
                    <p className="text-xs text-slate-400 dark:text-slate-500">Mes a mes</p>
                  </div>
                </div>
              ))
            )}
        ...
        <div className="bg-indigo-600 rounded-3xl p-8 text-white relative overflow-hidden shadow-xl shadow-indigo-100 flex flex-col justify-between min-h-[320px]">
          ...
          <div className="flex flex-col gap-3 relative z-10">
            <button
              ...
              className="w-full py-3 px-6 bg-white dark:bg-slate-900 text-indigo-600 dark:text-indigo-400 font-bold rounded-2xl hover:bg-indigo-50 dark:hover:bg-slate-800 transition-colors flex items-center justify-center gap-2"
            >
